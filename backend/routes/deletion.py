...
async def initiate_deletion(
    ...
    email_sent = await _send_deletion_email(email, result.get("confirmation_token"))
...
async def _send_deletion_email(email: str, token: str | None) -> bool:
    if not token:
        return False
...
@router.post("/resend")
async def resend_deletion_email(_user: dict = Depends(require_auth)):
    """Reenvia o e-mail de confirmação da exclusão pendente ('pending_confirmation')."""
    user_id = _user.get("id") or _user.get("sub")
    email = _user.get("email")
    if not user_id or not email:
        raise HTTPException(status_code=400, detail="User info incomplete")
    try:
        ...
        r = (get_admin_client().table("user_deletion_requests")
             .select("confirmation_token,status")
             .eq("user_id", user_id).eq("status", "pending_confirmation")
             .order("created_at", desc=True).limit(1).execute())
    except Exception as e:
        logger.error(f"resend_deletion lookup failed: {e}")
        raise HTTPException(503, "Não foi possível reenviar agora.")
    if not r.data:
        raise HTTPException(404, "Nenhuma exclusão aguardando confirmação.")
    email_sent = await _send_deletion_email(email, r.data[0].get("confirmation_token"))
