...
async def get_audit_log(
    ...
    offset = (page - 1) * limit
    action_filter = f'&action=ilike.*{action}*' if action else ''
    try:
        async with httpx.AsyncClient(timeout=8.0) as c:
            r = await c.get(
                f'{SUPABASE_URL}/rest/v1/admin_audit_events?select=id,actor_id,action,target_id,after,correlation_id,created_at&order=created_at.desc&limit={limit}&offset={offset}{action_filter}',
