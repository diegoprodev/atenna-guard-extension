/**
 * Ícones — Lucide (MIT · https://lucide.dev). Um traço, um peso, 24×24.
 * Fonte: devDependency `lucide-static`. Estes são os *inner paths* dos ícones
 * que a extensão usa — nada de SVG desenhado à mão.
 * Regenerar / adicionar: `node scripts/gen-icons.mjs`
 */
...
export interface IconOpts {
  ...
  /** classe(s) CSS no <svg> */
  cls?: string;
  /** preenche o traço (ex.: estrela favoritada) */
  fill?: boolean;
}

/** Retorna o SVG do ícone Lucide como string, pronto pra innerHTML. */
export function icon(name: IconName, opts: IconOpts = {}): string {
  const { size = 16, stroke = 2, cls = '', fill = false } = opts;
  return (
    `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" ` +
    `viewBox="0 0 24 24" fill="${fill ? 'currentColor' : 'none'}" stroke="currentColor" ` +
    `stroke-width="${stroke}" stroke-linecap="round" stroke-linejoin="round"` +
    `${cls ? ` class="${cls}"` : ''} aria-hidden="true">${PATHS[name]}</svg>`
  );
}
