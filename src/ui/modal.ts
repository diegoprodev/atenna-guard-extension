...
const YEARLY_PRICE  = 197.00;
const YEARLY_MONTHLY_EQUIV = YEARLY_PRICE / 12;                          // ~16,41
const YEARLY_SAVINGS = (MONTHLY_PRICE * 12) - YEARLY_PRICE;             // 161,80
const YEARLY_SAVINGS_PCT = Math.round((YEARLY_SAVINGS / (MONTHLY_PRICE * 12)) * 100); // ~45%

function renderPlansModal(trigger: string): void {
...
function renderPricingCards(container: HTMLElement, source: string): void {
  const wrap = document.createElement('div');
...
async function openCheckout(source: string, btn?: HTMLButtonElement, plan: 'yearly' | 'monthly' = 'yearly'): Promise<void> {
